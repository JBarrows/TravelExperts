<?php /**********************************************
        Created on 2018/05/31 by Joel Barr
        for Travel Experts Threaded Project
        PROJ-207-B  OOSD Spring 2018, SAIT
****************************************************/?>

<footer class='container-fluid row m-0'>
    <div class='text-md-left col socialicons'>
        <i class="fab fa-twitter-square"></i>
        <i class="fab fa-facebook-square"></i>
        <i class="fab fa-instagram"></i>
        <i class="fab fa-tumblr-square"></i>
    </div>
    <div class='text-center col-md-6 col-xl-8'>
        <p>&copy;SAIT, One Media, and Travel Experts Inc.</p>
        <p>
            "<a href="https://flic.kr/p/cTsU15" target="_blank">Sunrise view from a plane</a>"
            by <a href="https://www.flickr.com/photos/frankfarm/" target="_blank">Frank Farm</a>
            is used under the <a href="https://creativecommons.org/licenses/by-nc-nd/2.0/legalcode" target="_blank">cc by-nc-nd 2.0 license</a>.
            All other the <a href='https://creativecommons.org/publicdomain/zero/1.0/'>CC0 1.0 Universal license</a>
        </p>
    </div>
    <div class='col'></div>
</footer>
